...
def create_room(data):
    ...
    if data["rName"] not in ROOMS:
        ...
        SOCKETIO.emit('couldNotCreate', {"room": data["rName"]},
                      broadcast=False,
                      room=data['rName'])
        SOCKETIO.emit('onRoom',
                      ROOMS[data["rName"]]['activeUsers'],
                      broadcast=False,
                      room=data['rName'])
    else:
        print(rooms())
        SOCKETIO.emit(
            'couldNotCreate',
            'Room Already Exists Please Join Room or Create a New Room',
            room=rooms()[0])
...
def joining_room(data):
    ...
    if data["rName"] in ROOMS:
        ...
        print(rooms())
        SOCKETIO.emit('couldNotCreate',
                      'Room Does Not Exist Please Create Room',
                      room=rooms()[0])
...
def on_submit(data):
    ...
    for keys in ROOMS[data["room"]]['genreVotes']:
        if votes[counter] == '1' and votes[counter] is not None:
            ROOMS[data["room"]]['genreVotes'][keys][0] = ROOMS[
                data["room"]]['genreVotes'][keys][0] + 1
        counter = counter + 1
    #print(ROOMS[data["room"]]['genreVotes'])
    print(data["room"], 'line199')
    print(winning_genre(ROOMS[data["room"]]['genreVotes']),
          'The winning genres line 200')
    SOCKETIO.emit('onAdminSubmit',
                  ROOMS[data["room"]]['genreVotes'],
                  broadcast=True,
                  room=data["room"])
...
def on_submit_movie_votes(data):
    ...
    for keys in ROOMS[data['room']]['movieVotes']:
        if votes[counter] == '1' and votes[counter] is not None:
            ROOMS[data['room']]['movieVotes'][keys][0] = ROOMS[
                data['room']]['movieVotes'][keys][0] + 1
...
def get_movies(data):
    ...
    for i in range(10):
        ...
        ROOMS[data]["movieVotes"][movie_response['results'][i]
                                  ['original_title']] = []
        ROOMS[data]["movieVotes"][movie_response['results'][i]
                                  ['original_title']].append(0)
        ROOMS[data]["movieVotes"][
            movie_response['results'][i]['original_title']].append(
